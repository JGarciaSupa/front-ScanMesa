...
export default function KDSPage() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-start gap-3 md:gap-4 w-full pb-6">
      {orders.length === 0 ? (
        <div className="col-span-full flex flex-col items-center justify-center text-muted-foreground gap-4 mt-20">
        ...
        orders.map((order) => {
          ...
          return (
            <Card
              key={order.id}
              className={`py-0 flex flex-col overflow-hidden shadow-md transition-all duration-300 ${colors.cardClass}`}
            >
              <CardHeader className={`px-3 py-2.5 md:px-4 md:py-3 flex flex-row items-center justify-between space-y-0 gap-2 shrink-0 ${colors.headerClass}`}>
                <div className="flex flex-col gap-0.5">
                  <span className="text-xl md:text-2xl font-black uppercase tracking-widest leading-none">
                    {order.table}
                  </span>
                  {order.diner && (
                    <span className="text-xs md:text-sm font-bold opacity-90 uppercase tracking-wider">
